...
const string = "Heyo! How's it going?";

/*
  Determine what script a character belongs to.
  Count all the characters and map them to the scripts.
  Create readable string from mapped list.
*/

const characterScript = (char = "") => {
  for (let script of SCRIPTS) {
    const charInScript = script.ranges.some((range) => {
      const [from, to] = range;
...
const characterCount = (str) => {
  ...
  for (let char of str) {
    const charScript = characterScript(char);

    if (charScript) {
      const scriptIndex = count.findIndex((script) => script.name === charScript.name);
...
const formatCharacterCount = (charCounts) => {
  const total = charCounts.reduce((acc, curr) => acc + curr.count, 0);

  const formattedCharCounts = [];
  for (let charCount of charCounts) {
    formattedCharCounts.push(`${Math.round((charCount.count * 100) / total)}% ${charCount.name}`);
  }

  return formattedCharCounts.join(", ").trim();
};

console.log(formatCharacterCount(characterCount("Ow 英国的狗说")));
